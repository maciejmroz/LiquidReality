...
#else
      LPCTSTR pszText = _T("<< Click here to add a new item >>");
#endif
      CDCHandle dc(di.hDC);
...
public:
   ...
   void SetExtendedGridStyle(DWORD dwExtStyle)
   {
      ...
      if( (m_di.dwExtStyle & PGS_EX_ADDITEMATEND) && !(dwExtStyle & PGS_EX_ADDITEMATEND) ) {
         // Remove last item
         DeleteItem(TBase::GetItemCount()-1);
      } 
      else if( dwExtStyle & PGS_EX_ADDITEMATEND ) {
         // Add last item
   ...
   {
      if( m_di.dwExtStyle & PGS_EX_ADDITEMATEND ) return TBase::GetItemCount()-1;
   ...
   BOOL GetItemText(HPROPERTY hProp, LPTSTR pszText, UINT cchMax) const
   {
      ATLASSERT(::IsWindow(m_hWnd));
      ATLASSERT(hProp);
      ATLASSERT(!::IsBadWritePtr(pszText,cchMax));
      if( hProp == NULL || pszText == NULL ) return FALSE;
      return hProp->GetDisplayValue(pszText, cchMax);
   }
   ...
   {
      return (m_di.dwExtStyle & PGS_EX_ADDITEMATEND) && 
         ...
         if( ::SendMessage(GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM) &nmh) == 0 ) {
            ...
            if( !bIsLastAddItem ) {         
               ...
               if( prop->IsEnabled() ) {
                  ...
                  if( m_di.dwExtStyle & PGS_EX_SINGLECLICKEDIT ) fActivate = true;
                  if( fActivate ) {
   ...
   {      
      ...
      case VK_RETURN:
         ...
         return 0;
      case VK_TAB:
         if( m_di.dwExtStyle & PGS_EX_TABNAVIGATION ) {
   ...
   LRESULT OnSelChanged(int /*idCtrl*/, LPNMHDR pnmh, BOOL& bHandled)
   {
      ...
      // Only items with selected changes...
   ...
   {
      if( m_di.dwExtStyle & PGS_EX_NOSHEETNAVIGATION ) return 0;
   ...
   LRESULT OnHeaderChanging(int /*idCtrl*/, LPNMHDR /*pnmh*/, BOOL& bHandled)
   {
      ...
      if( m_di.dwExtStyle & PGS_EX_ADDITEMATEND ) {
         RECT rcItem = { 0 };
   ...
   void DrawItem(CDCHandle dc, int iItem, int iSubItem, UINT iState)
   {
      ...
      if( iItem == m_iSelectedRow ) {
         ...
         if( (m_di.dwExtStyle & PGS_EX_FULLROWSELECT) ) {
         ...
         if( iSubItem == m_iSelectedCol ) {
            ...
            if( m_di.dwExtStyle & PGS_EX_INVERTSELECTION ) {
      ...
      if( (::GetFocus() == m_hWnd && (iState & CDIS_FOCUS)) || (dwStyle & LVS_SHOWSELALWAYS) ) {
         if( (iItem == m_iSelectedRow) && (iSubItem == m_iSelectedCol || bIsLastItem ) ) {
            if( iItem != m_iInplaceRow && iSubItem != m_iInplaceCol ) {
               if( GetExtendedListViewStyle() & LVS_EX_GRIDLINES ) {
